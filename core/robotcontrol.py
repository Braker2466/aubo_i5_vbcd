...
import time
#linux
from auboi5_lib import libpyauboi5
import logging
...
class Auboi5Robot:
    ...
    def create_context(self):
        """"
        每次调用 create_context() 都会生成一个新的RSHD
...
def move_rotate_test():
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            # 工具转轴的向量（相对于法兰盘，这样需要测量得到x,y,z本测试样例默认以x=0,y=0,ｚ轴为0.18米）
            tool_pos_on_end = (0, 0, 0.18)
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            while True:
                ...
                robot.move_joint(joint_radian, True)  # 轴动关节
